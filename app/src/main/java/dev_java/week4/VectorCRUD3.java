...
public class VectorCRUD3 {
  ...
  public void getDeptList() {
    for (int i = 0; i < vdept.size(); i++) {
      DeptVO dv = vdept.get(i);
      System.out.println(dv.getDeptno() + ", " + dv.deptname + ", " + dv.getLoc());
    } //// end of outter
  }
  ...
  public static void main(String[] args) {
    ...
    // 입력테스트
    DeptVO dvo1 = DeptVO.builder().deptno(10).deptname("개발부").loc("인천").build();
    // dvo1의 주소번지 ========> ① 2번과 다름
    int result1 = vCrud.deptInsert(dvo1);// 답
    dvo1 = null; // 기존에 참조하는 객체와 연결을 끊는다.
    dvo1 = DeptVO.builder().deptno(20).deptname("총무부").loc("서울").build();
    // dvo1의 주소번지 ========> ② 1번과 다름
