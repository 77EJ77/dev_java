...
public class DBConnectionMgr {
  ...
  public Connection getConnection() {
    Connection con = null;
    // 네트워크니까 장애 발생 가능성이 있어 예외처리 필수
    try {
      Class.forName(_DRIVER);
      con = DriverManager.getConnection(_URL, _USER, _PW);
    } catch (Exception e) {// 멀티
      System.out.println("드라이버 클래스를 찾을 수 없습니다.");
    }
    return con;
  }

  // 사용한 자원을 반납하는 코드는 명시적으로 하는 것을 원칙으로 하고있음
 ...
 * 4. SELECT문의 경우 오라클 서버에서 제공하는 커서를 지원하는 ResultSet인터페이스를 활용하여 테이블에 제공되는 커서를 조작하여
 * 해당 로우에 데이터가 존재하면 Cursor_open_fetch과정을 통해서 오라클 서버로부터 데이터를 꺼내서 List<DeptVO>에 담을
 * 수 있다.
 * 주의: INSERT, UPDATE, DELETE에서는 커서가 필요없음 - commit, rollback
 * 리턴값은 INT임
 * 이것들을 요청할 때는 executeUpdate(INSERT문 or UPDATE or DELETE문)
 * SELECT일때는
 * ResultSet rs = executeQuery("SELECT * FROM deot");
 * 오라클에서 제공되는 커서를 조작하는 인터페이스를 리턴 타입으로 제공받아서 조회결과를 Collection에 담을 수 있다.
 ...
 * SELECT인경우
 * ResultSet 먼저 닫고
 * Statement / PreparedStatement 두번째로 닫고
 * Connection을 가장 나중에 닫아주면 됨
 * 닫지 않으면 자동으로 언젠가 닫아주기는 하지만 명시적으로 닫아주면 닫는 시간을 앞당길 수 있어 플젝에서는 명시적으로 닫는 코드 작성을
 * 가이드로 정함
 * 
 * INSERT, UPDATE, DELETE는 두번째와 세번째만 닫으면 됨
 * 왜냐하면 리턴타입이 커서가 필요없으니까...
 * 
 * rs.previus / rs.isFirst / rs.~~~
 */
