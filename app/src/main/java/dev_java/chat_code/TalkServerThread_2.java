...
public class TalkServerThread_2 extends Thread {
	TalkServer ts = null;
	...
	//Room room = new Room();
	public TalkServerThread_2(TalkServer ts) {
		this.ts = ts;
		try {
			...
			for(TalkServerThread_2 tst:ts.globalList) {
         ...
         for(int i = 0; i < ts.roomList.size(); i++ ) {
            ...
            if(g_title.equals(room.title)) {
               for(int j = 0; j < room.userList.size(); j++ ){
                  TalkServerThread_2  tst = (TalkServerThread_2)room.userList.get(j);
                  try{
                       tst.send(msg);
                  }catch(Exception ex) {
	...
	public void roomCasting(String msg, String roomTitle) {
		for(int i=0;i<ts.roomList.size();i++) {
			...
			if(roomTitle.equals(room.title)) {
				for(int j=0;j<room.userList.size();j++) {
					TalkServerThread_2 tst = room.userList.get(j);
					try {
						tst.send(msg);
					} catch (Exception e) {
